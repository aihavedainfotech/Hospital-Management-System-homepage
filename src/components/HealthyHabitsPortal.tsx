...
const habits: Habit[] = [
  {
    id: 1, category: 'Nutrition', icon: 'fas fa-apple-alt', color: '#2BBF9C',
  ...
  {
    id: 2, category: 'Fitness', icon: 'fas fa-walking', color: '#2EA3F2',
...
const bmiCategories = [
  ...
  { range: '18.5 - 24.9', label: 'Normal', color: '#2BBF9C' },
...
function BMICalculator() {
  ...
  return (
    <div style={{ background: '#FFFFFF', borderRadius: '16px', padding: '1.5rem',
      border: '1px solid #E6EEF5', boxShadow: '0 4px 12px rgba(31, 45, 61, 0.05)',
      display: 'flex', flexDirection: 'column' }}>
      <h3 style={{ fontSize: '1.1rem', fontWeight: 600, color: '#1F2D3D', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <i className="fas fa-calculator" style={{ color: '#2EA3F2' }}></i>
        BMI Calculator
      </h3>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
        <input type="number" value={weight} onChange={e => setWeight(e.target.value)} placeholder="Weight (kg)"
          style={{ width: '100%', padding: '0.6rem', border: '1px solid #E6EEF5', borderRadius: '8px', background: '#F7FBFF', fontSize: '0.9rem', outline: 'none' }} />
        <input type="number" value={height} onChange={e => setHeight(e.target.value)} placeholder="Height (cm)"
          style={{ width: '100%', padding: '0.6rem', border: '1px solid #E6EEF5', borderRadius: '8px', background: '#F7FBFF', fontSize: '0.9rem', outline: 'none' }} />
      </div>
      <button onClick={calculate} style={{ background: '#2EA3F2', color: 'white', border: 'none', padding: '0.7rem', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem' }}>
        Calculate
      </button>
      {bmi !== null && (
        <div style={{ textAlign: 'center', marginTop: '1rem', background: '#F7FBFF', borderRadius: '12px', padding: '1rem' }}>
          <div style={{ fontSize: '2rem', fontWeight: 700, color: getBmiColor(bmi) }}>{bmi}</div>
          <div style={{ fontSize: '0.85rem', color: '#6B7C93' }}>Your BMI</div>
        </div>
      )}
    </div>
  );
...
export default function HealthyHabitsPortal() {
  ...
  return (
    <section id="healthy-habits" className="section-pad" style={{
      ...
      <style>{`
        .hab-card {
          background: #FFFFFF; border: 1px solid #E6EEF5; border-radius: 16px; padding: 1.5rem;
          transition: all 0.2s ease; cursor: pointer;
        }
        .hab-card:hover {
          transform: translateY(-4px); border-color: #2EA3F2;
          box-shadow: 0 8px 24px rgba(31, 45, 61, 0.08);
        }
        .hab-chip {
          padding: 6px 14px; border-radius: 20px; border: 1px solid #E6EEF5;
          background: #F7FBFF; color: #6B7C93; font-size: 0.8rem; font-weight: 500;
          cursor: pointer; transition: all 0.2s;
        }
        .hab-chip.active {
          background: #2EA3F2; color: white; border-color: #2EA3F2;
        }
      `}</style>

      <div className="container">
        <div ref={head.ref} className={head.className} style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <h2 className="section-title" style={{ color: '#1F2D3D' }}>Healthy Habits</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
          ...
          <div style={{ background: '#F7FBFF', borderRadius: '16px', padding: '1.5rem', border: '1px solid #E6EEF5' }}>
            ...
            <p style={{ fontSize: '0.85rem', color: '#6B7C93', marginBottom: '1rem' }}>Track your daily 8 glasses of water for optimal health.</p>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'center' }}>
              {[1,2,3,4,5,6,7,8].map(i => (
                <div key={i} style={{ width: '32px', height: '32px', borderRadius: '8px', background: '#FFFFFF', border: '1px solid #E6EEF5', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#E6EEF5' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #2EA3F2, #2BBF9C)', borderRadius: '16px', padding: '1.5rem', color: 'white' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.5rem' }}>
            {filtered.map((habit) => (
              <div key={habit.id} className="hab-card" onClick={() => setSelectedHabit(habit)}>
                ...
                <h3 style={{ fontSize: '1rem', fontWeight: 600, color: '#1F2D3D', marginBottom: '8px' }}>{habit.title}</h3>
                <p style={{ fontSize: '0.85rem', color: '#6B7C93', lineHeight: 1.5 }}>{habit.summary}</p>
              </div>
            ))}
          </div>
        </div>
